...
def create_weather_card_sync(weather_data, output_path):
    ...
    try:
        ...
        emoji_img = get_weather_emoji(weather_data['description'], current_time_local)
        ...
        draw.text((325, 170), f"{city_name}, {weather_data['country']}", font=FONT_LARGE, fill=city_color)
        ...
        draw.text((230, 265), current_time_str, font=FONT_MEDIUM, fill=date_color)
        ...
        draw.text((right_block_x, 390), f"{weather_data['pressure']} мм | {weather_data['humidity']}%", font=FONT_MEDIUM, fill=right_color)
        draw.text((right_block_x, 460), f"{weather_data['wind_speed']} м/с, {weather_data['wind_dir']}", font=FONT_MEDIUM, fill=right_color)
        draw.text((right_block_x, 530), f"ощущается {weather_data['feels_like']:+.1f}°".replace('.', ','), font=FONT_MEDIUM, fill=right_color)

        draw.text((230, 630), weather_data['description'].capitalize(), font=FONT_LARGE, fill=description_color)

        sunrise_time = weather_data['sunrise']
        sunset_time = weather_data['sunset']
        
        draw.text((230, 728), f"Восход {sunrise_time} | Закат {sunset_time}", font=FONT_LARGE, fill=sunrise_color)
